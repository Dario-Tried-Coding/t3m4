...
export const T3M4Provider = <Sc extends Schema, C extends Config<Sc>>({ children, ...scriptArgs }: T3M4ProviderProps<Sc, C>) => {
  ...
  useEffect(() => {
    ...
    window.T3M4.subscribe('State:Computed:Update', 'React:State:Update', ({ state: computed, colorScheme }) => {
      setState((state) => ({ ...state, computed: computed as T3M4Context<Sc, C>['state']['computed'] }))
      setColorSchemes((colorSchemes) => ({ ...colorSchemes, computed: colorScheme as T3M4Context<Sc, C>['colorSchemes']['computed'] }))
    })
