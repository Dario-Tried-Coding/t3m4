...
export default async function Page(props: Props) {
  ...
  return (
    <Pump queries={[{ docs: { item: ArticleFragmentRecursive, __args: { filter: { _slug: { eq: slugs.at(0) } } } } }]} bind={{ slug: `/${slugs.join('/')}` }}>
      {async ({ slug }, [{ docs }]) => {
        'use server'
        ...
        return <pre>{JSON.stringify({ slug, article }, null, 2)}</pre>
      }}
    </Pump>
  )
