import { articleFragment, flattenArticles, slugsFragment } from '@/helpers/basehub'
...
export const generateStaticParams = async () => {
  const { docs } = await basehub({ cache: 'no-store' }).query({
...
export default async function Page(props: Props) {
  ...
  return (
    <Pump
      ...
      bind={{ slug: `/docs/${slug?.join('/') ?? 'introduction'}` }}
      draft={(await draftMode()).isEnabled}
    >
      {async ({ slug }, [{ docs }]) => {
        'use server'

        return <pre>{JSON.stringify({ slug, docs }, null, 2)}</pre>

        // const article = findArticleBySlug(docs.items, slug) as fragmentOn.infer<typeof articleFragment> | undefined
