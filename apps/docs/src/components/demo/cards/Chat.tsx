      ...
      <Card>
        ...
        <CardContent>
          <div className='flex flex-col gap-4'>
            {messages.map((message, index) => (
              <div key={index} className={cn('flex w-max max-w-[75%] flex-col gap-2 rounded-lg px-3 py-2 text-sm', message.role === 'user' ? 'bg-primary text-primary-foreground ml-auto' : 'bg-muted')}>
                {message.content}
              </div>
            ))}
          </div>
        ...
        <CardFooter>
          ...
          >
            ...
            <Button type='submit' size='icon' className='absolute right-2 top-1/2 size-6 -translate-y-1/2 rounded-full' disabled={inputLength === 0}>
