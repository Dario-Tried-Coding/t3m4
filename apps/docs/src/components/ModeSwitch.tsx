...
export const ModeSwitch: FC = ({}) => {
  const { state, setState } = useT3M4('root')
  const t = useTranslations('ThemeSwitch')

  return (
    <Button variant='ghost' size='icon' disabled={!state.computed || !!state.forced?.mode} onClick={() => setState({ mode: state.computed?.mode === 'light' ? 'dark' : 'light' })}>
