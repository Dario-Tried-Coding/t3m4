...
;(() => {
  ...
  var Engine = class _Engine {
        ...
        deep: {
          state: {
            maps: {
              ...
              all(...sources) {
                const islandStates = sources.reduce((islandStates2, source) => {
    ...
    constructor() {
      ...
      this.facets = Object.entries(schema).reduce((acc, [i, { mode, facets }]) => {
        const obj = {}
        obj.facets = Object.keys(facets ?? {}).reduce((acc2, facet) => acc2.add(facet), /* @__PURE__ */ new Set())
        if (mode) obj.mode = true
        return acc.set(i, obj)
      }, /* @__PURE__ */ new Map())
      this.values = Object.entries(schema).reduce((acc, [i, { facets, mode }]) => {
        const islandValues = {}
        if (facets)
          islandValues.facets = Object.entries(facets).reduce((acc2, [f, v]) => {
      ...
      this.fallbacks = Object.entries(_Engine.args.config).reduce((acc, [i, { facets, mode }]) => {
        const islandFallbacks = {}
        if (facets)
          islandFallbacks.facets = Object.entries(facets).reduce((acc2, [f, stratObj]) => {
      ...
      this.storage = {
        ...
        toStore: Object.entries(_Engine.args.config).reduce((acc, [i, { mode, facets }]) => {
          const islandObj = {}
          if (mode) islandObj.mode = _Engine.args.config[i].mode.store ?? PRESET.storage.store.value
          if (facets) {
            const facetsToStore = Object.entries(facets).reduce((acc2, [facet, stratObj]) => {
      ...
      this.selectors = {
        ...
        observe: {
          dataAttributes: {
            island: 'data-island',
            forced: Object.entries(schema).reduce((acc, [i, { facets, mode }]) => {
              if (facets) Object.keys(facets).forEach((f) => acc.add(`data-force-${i}-facet-${f}`))
              if (mode) acc.add(`data-force-${i}-mode`)
              return acc
            }, /* @__PURE__ */ new Set()),
            computed: Object.entries(schema).reduce((acc, [, { facets, mode }]) => {
      ...
      this.modes = Object.entries(schema).reduce((acc, [i, { mode }]) => {
        if (!mode) return acc
        const stratObj = _Engine.args.config[i].mode
        const obj = {
          selectors: /* @__PURE__ */ new Set([
            ...PRESET.modes.dom.selectors,
            ...(_Engine.args.selector ? (typeof _Engine.args.selector === 'string' ? [_Engine.args.selector] : _Engine.args.selector) : []),
          ...
          colorSchemes:
            stratObj.strategy === STRATS.mono
              ? /* @__PURE__ */ new Map([[stratObj.default, stratObj.colorScheme]])
              : stratObj.strategy === STRATS.multi
                ? new Map(Object.entries(stratObj.colorSchemes))
                : new Map([[mode.light, COLOR_SCHEMES.light], [mode.dark, COLOR_SCHEMES.dark], ...Object.entries(stratObj.colorSchemes ?? {})]),
          system: void 0,
        }
  ...
  var StorageManager = class _StorageManager {
    ...
    static constructStateToStore(state) {
      const stateToStore = Array.from(state).reduce((acc, [i, { mode, facets }]) => {
        ...
        if (facets) {
          islandState.facets = Array.from(facets).reduce((facetsAcc, [facet, value]) => {
  ...
  var DomManager = class _DomManager {
    ...
    static get = {
      islands: {
        ...
        all() {
          const elements = Array.from(Engine.getInstance().islands).reduce((acc, island) => {
        ...
        computed: {
          island: {
            dirty: (island, el) => {
              ...
              if (facets && facets.size > 0) {
                const facetsMap = Array.from(facets).reduce((acc, facet) => {
          ...
          all: {
            dirty: () => {
              ...
              const states = Array.from(islands).reduce((states2, [i, els]) => {
                const islandStates = Array.from(els).reduce((islandStates2, el) => {
        ...
        forced: {
          island: {
            dirty: (island) => {
              ...
              const state = {}
              if (facets)
                state.facets = Array.from(facets).reduce((facets2, facet) => {
            ...
            sanitized: (island) => {
              ...
              if (dirty.facets && dirty.facets.size > 0)
                sanitized.facets = Array.from(dirty.facets).reduce((facets, [facet, value]) => {
    ...
    static set = {
      ...
      mode: {
        ...
        all: (island, value, opts) => {
          if (Engine.getInstance().modes.get(island)?.selectors.has('data-color-scheme')) _DomManager.set.mode.dataAttribute(island, value, opts)
    ...
    constructor() {
      ...
      EventManager.on('State:Computed:Update', 'DomManager:State:Update', ({ state }) => _DomManager.set.state.computed.all(Engine.utils.convert.deep.state.objToMap(state)))
      ...
      media.addListener((e) => {
        ...
        currModes?.forEach((mode, island) => {
          ...
          if (isSystemStrat && isSystemMode) {
            ...
            _DomManager.set.mode.all(island, colorScheme ?? fallbackColoScheme)
          }
        })
      })
    }
