      ...
      state: {
        computed: {
          island: (island, state, opts) => {
            const enableBackTransitions = Engine.getInstance().disableTransitionOnChange ? _DomManager.disableTransitions() : void 0
          ...
          all: (state, elements) => {
            const els = elements ?? _DomManager.get.islands.all()
            els.forEach((islandEls, island) => {
              const islandState = state.get(island)
              _DomManager.set.state.computed.island(island, islandState, { elements: islandEls, isAlreadyMutatingDom: true })
            })
          },
        },
      },
